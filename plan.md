...
- [x] **Relation-authoring dialog** — the scenario add-change dialog now offers `add_relation` (source `CardPicker` → relation-type select filtered by endpoint types → target `CardPicker`) and `remove_relation` (pick a card → choose one of its existing relations), so relation changes are authorable in the UI, not just via API.
- [ ] **Remaining:** LDV dashed/NEW overlay render, approval-workflow UI polish, docs/screenshots.
...
9. [~] **3.1.1** Scenario Planning & Transition Architecture — models + migration + permissions + diff + merge + view + i18n + tests + workspace transfer + relation-level changes + baseline-drift detection + relation-authoring dialog done; LDV dashed/NEW overlay + approval-UI polish + docs pending
