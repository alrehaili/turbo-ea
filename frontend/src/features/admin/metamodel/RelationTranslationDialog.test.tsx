...
const RELATIONS = [
  {
    ...
    attributes_schema: [],
    translations: {
      label: { en: "offers", ar: "يعرض" },
      reverse_label: { en: "is offered by", ar: "معروض من" },
    },
  },
  {
    key: "relAppToApp",
...
describe("RelationTranslationDialog", () => {
  ...
  it("offers no English tab", async () => {
    ...
    expect(await screen.findByRole("tab", { name: /العربية/ })).toBeInTheDocument();
  ...
  it("counts completeness per locale, ignoring English", async () => {
    ...
    // Arabic has 2 of the 3 verbs translated.
    expect(await screen.findByRole("tab", { name: /العربية.*2\/3/ })).toBeInTheDocument();
  });
  ...
  it("saves only the relations whose translations changed", async () => {
    ...
    expect(body.translations.label).toEqual({ en: "offers", ar: "يقدم" });
  ...
  it("never sends translations: null (the column is NOT NULL)", async () => {
    ...
    // Clear the only Arabic verb on the first relation's row.
    const inputs = await screen.findAllByRole("textbox");
    await user.click(screen.getByRole("tab", { name: /العربية/ }));
    const arabicInputs = await screen.findAllByRole("textbox");
    await user.clear(arabicInputs[0]);
    await user.click(screen.getByRole("button", { name: /^save$/i }));
