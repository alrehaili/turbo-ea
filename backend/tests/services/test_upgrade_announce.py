...
class TestReadWhatsNew:
    async def test_reports_the_announced_span(self, db, monkeypatch):
        # Fork versions: the bundled CHANGELOG is fork-only, so these must be
        # headings that actually exist in it.
        monkeypatch.setattr(upgrade_announce, "APP_VERSION", "2.27.0")
        await _settings(db, **{ANNOUNCED_FROM_KEY: "2.25.0"})

        out = await read_whats_new(db)

        assert out["version"] == "2.27.0"
        assert out["from_version"] == "2.25.0"
        # Real bundled changelog: the span must include the current version.
        assert "## 2.27.0" in out["notes"]
        assert "## 2.25.0" not in out["notes"]
