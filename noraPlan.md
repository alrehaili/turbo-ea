...
| 100 | User backlog (owner-reported gaps, fix later) | WP100.1–WP100.2 | ☐ recorded 2026-07-12 |
...
### WP100.1 — Pillar surfacing beyond the two strategy reports `☐`
...
- [ ] **Inventory & counts**: verify Pillar appears in the inventory type filter, card counts, and the Create-card dialog after the v6 apply (should be automatic — the metamodel is data-driven — but confirm and screenshot).
- [ ] **Business layer overview** (`/layers/business`): confirm Pillar cards appear in the layer's card-type breakdown and swim-lane (category = Business); add the type to any hardcoded layer-order/type lists if it's missing.
- [ ] **NORA executive dashboard** (`/nora-program`): consider a small strategy tile (pillars / objectives / unaligned count) deep-linking to the Strategy Cascade.
- [ ] **Card detail affordances**: the Objective card's Relations section should offer the *supports → Pillar* relation naturally (CardPicker on the relation type — verify it lists Pillar cards); the Pillar card detail shows its supporting objectives via the reverse relation.
- [ ] **Strategic House / Cascade cross-links**: from a Pillar card, a deep-link chip to the Strategy Cascade filtered view; from the reports, an "add pillar" empty-state button (Create-card dialog pre-set to Pillar).
- [ ] **Search & LDV**: confirm global search and the dependency views render Pillar with its icon/color (`foundation` / `#7b1fa2`); add the color to `CARD_TYPE_COLORS` tokens if the LDV falls back to grey.
- [ ] Demo/docs: mention Pillar in the fork docs pass; the demo already seeds two pillars.

### WP100.2 — Saudi government organizational hierarchy `☐`
...
- [ ] **Profile v7 pass**: append Organization subtypes `sector`, `generalDepartment`, `department`, `sectionUnit` — idempotent by key, translated ×9 with the Arabic names above first-class. Existing seeded subtypes stay (additive, like the ITComponent NEA subtypes precedent in WP6.3).
- [ ] **Org Chart report** (`/reports/org-chart`): already renders subtype labels per node — verify the four new subtypes show correctly; consider level-colored icons per subtype (sector emphasised) and an optional expected-order lint (a `department` directly under `sector` is legal but flaggable as "skipped level" — advisory only, never a hard block).
- [ ] **حصر البيانات importer**: check whether the بنية الأعمال workbook's organizational-unit sheet carries a level/type column; if so, map it onto the new subtype keys in the `nora_xlsx` adapter.
- [ ] **Strategy tie-in**: Organizations own programs/initiatives via the existing Organization↔Initiative relation — consider showing the owning unit as a chip in the Strategy Cascade rows.
- [ ] Demo data: re-model the NexaTech-style NORA demo org tree (`org_root` ⊃ deputyships) onto the four levels so the org chart demos the full ladder.
