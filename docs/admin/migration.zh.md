...
> 当前支持的源平台：**SAP LeanIX** 和 **NORA 数据收集模板（DGA）**。其他适配器（Ardoq、Mega HOPEX、BiZZdesign、Avolution Abacus 等）接入相同的暂存与应用流水线，发布后会自动出现在上传对话框中。
...
导入器还可读取沙特国家 EA 框架的官方数据收集工作簿——每个 EA 领域一个工作簿（业务、应用、技术、安全、数据）。各工作表落到对应的卡片类型，选项列会从模板自带的查找值翻译为 NORA 配置文件字段——因此请在导入前启用 NORA 框架配置文件。

行之间的名称引用会转换为关系和层级链接；被引用但没有自己行的名称会创建为最小占位卡片，并绑定到同名的现有卡片。工作簿可按任意顺序上传并安全地重复导入。受益人体验改进、表单、政策与相关方工作表暂不导入。
