...
Каждая колонка `rel:<тип_связи>` выражает исходящие связи в виде списка целей через запятую. Ячейки **декларативны**: содержимое заменяет полный набор исходящих связей данного типа от источника. Удаление цели из списка удаляет связь; пустая ячейка удаляет все.
